...
```bash
git clone https://github.com/<你的用户名>/pdf-rescue-mcp.git
cd pdf-rescue-mcp
...
| 层级 | 机制 | 参数 |
|------|------|------|
| **业务层** | 独立 OCR 子进程、逐页缓存、原子 `状态.json` / JSONL | 页数、进度、速度、ETA、质量证据 |
| **监管层** | SQLite WAL 任务账本、fencing lease、心跳与页级前进、`psutil` 进程树收尾 | `WATCH_INTERVAL=5s`, `HEARTBEAT_TIMEOUT=90s`, `PROGRESS_TIMEOUT=600s` |
...
适用于不能拉起 stdio 子进程的 MCP 客户端。仅绑定回环地址：
...
- 常规批处理保持保守线程预算；容量基准可实测 2/4/6/8 线程与多 worker 组合
...
### Q: 为什么速度一直是 30-40秒/页？
A: CPU 模式下这是正常速度。可尝试 `book-fast` 模式（DPI=180），或安装 NVIDIA GPU 扩展。

### Q: 子进程频繁重启怎么办？
A: 检查 `logs/` 下的错误日志。常见原因：内存不足、PDF 损坏（先用 `diagnose_pdf` 检查）。
